...
| AI access | **DeepInfra by default, through the official OpenAI SDK only**, configured via the SDK's native `OPENAI_BASE_URL`/`OPENAI_API_KEY`; every model ID is env config; nothing provider-specific in code (D11). Each model role (`EMBEDDING_`/`OCR_`/`LLM_` `_BASE_URL`/`_API_KEY`) can independently override the shared pair, so a different OpenAI-compatible provider can serve any one role without affecting the others. Model IDs are initial preferences — only the embedding model is costly to change (re-index) |
| Prompts | **LLM prompt text is never embedded in source code** (owner rule 2026-07-27): every prompt is a Handlebars template — `worker/prompts/*.hbs` rendered at runtime (e.g. the catalog-description digest), `converter/prompts/ocr-image.hbs` static, fed to MarkItDown's `llm_prompt` (D16) |
