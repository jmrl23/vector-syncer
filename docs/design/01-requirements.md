...
- All AI inference (embeddings, OCR, catalog descriptions) is delegated to **DeepInfra** via its OpenAI-compatible API — an accepted data-sharing decision (D11). **Models and endpoint are configuration, never code**: only the official OpenAI SDK is used, pointed by `OPENAI_BASE_URL`/`OPENAI_API_KEY`, with every model ID in its own env var — any OpenAI-compatible provider (or a self-hosted bge-m3 sidecar, D4) slots in without touching code.
