...
| **AI provider** | **DeepInfra** (OpenAI-compatible API) | — | One key, one dashboard for all three models below; accessed **exclusively through the official OpenAI SDK** via its native `OPENAI_BASE_URL`/`OPENAI_API_KEY` env vars — models and endpoint are pure config, nothing hardcoded |
